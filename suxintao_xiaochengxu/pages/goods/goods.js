...
``
  ...
  onAddToCart:function(e){
    ...
    var page = this;
    app.request({
      url: 'v2/ecapi.cart.add',
      data: { product: this.data.goodsInfo.id, amount: this.data.buy_num, property:JSON.stringify(property) },
      success: function (res) {
        page._refreshCartNum();
        wx.showToast({
          title: '已加入购物车',
          success: function(res){
            setTimeout(function(){
  ...
  onBuyNow:function(e){
    ...
    wx.navigateTo({
      url: '/pages/checkout/checkout?onestep=1',
    })
  },

  onMinus: function (e) {
    var num = Math.max(1, this.data.buy_num - 1);

    if (this.data.goodsInfo.is_become_vip_goods){
      num = 1;
    }

    this.setData({
      buy_num: Math.max(1, num),
    });
  },

  onPlus: function (e) {
    var num = Math.min(this.data.stock, this.data.buy_num + 1);
    if (this.data.goodsInfo.is_become_vip_goods) {
      num = 1;
    }
    this.setData({
  ...
  onSelectAttr:function(e){
    ...
    // console.log(proIndex, attrIndex)
